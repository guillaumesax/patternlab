  ...
  static createMidiFile(tempo: number, drumGrid: boolean[][], chords: ChordItem[]) {
    ...
    chords.forEach((chord, idx) => {
      ...
      intervals.forEach((interval) => {
        chordNotesEvents.push({
          tick: startTick,
          status: 0x90,
          data: [basePitch + interval, 0x50],
        });
        chordNotesEvents.push({
          tick: endTick,
          status: 0x80,
          data: [basePitch + interval, 0x00],
        });
      });
    });
...
const Button = ({ children, onClick, variant = "primary", icon: Icon, className = "" }: any) => {
  const baseStyle =
    "flex items-center justify-center px-5 py-3 rounded-lg font-bold transition-all text-sm active:scale-[0.98] shadow-lg whitespace-nowrap";
      ...
      if (activeTabRef.current === "drums") {
        ...
        if (chordProgressionRef.current.length > 0 && step % 16 === 0) {
          ...
          if (chord) {
            ...
            intervals.forEach((interval) => {
              audio.playNote(
                { pitch: basePitch + interval, startTime: 0, duration: 16, velocity: 80 },
    ...
    <div className="min-h-screen bg-[#0d1117] text-gray-200 flex flex-col font-sans overflow-x-hidden">
      <header className="h-16 border-b border-gray-800/50 flex items-center px-5 bg-[#0d1117]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">
              générateur midi par Guillaume Sax
            </p>
          </div>
        </div>
        ...
        <div className="ml-auto">
          <div
            className={`flex items-center gap-2 text-[10px] font-bold px-3 py-1.5 rounded-full border ${
              isPlaying
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${
                isPlaying ? "bg-green-500 shadow-[0_0_8px_#22c55e]" : "bg-gray-600"
              }`}
            ></div>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto pb-10">
              ...
              <div className="flex items-center gap-8">
                <button
                  ...
                  className={`w-16 h-16 rounded-full flex items-center justify-center transition-all flex-shrink-0 ${
                    isPlaying ? "bg-red-600 shadow-red-600/30" : "bg-cyan-500 shadow-cyan-500/30"
                  } active:scale-90`}
                >
                  {isPlaying ? (
                    <Square fill="white" size={22} className="text-white" />
                  ) : (
                    <Play fill="white" className="ml-1 text-white" size={26} />
                  )}
                </button>
        ...
        <div className="px-4 mt-6 space-y-4">
          <div className="flex bg-[#161b22] p-1.5 rounded-xl border border-gray-800/50">
            {[
              { id: "drums", label: "Batterie", icon: Volume2 },
              { id: "chords", label: "Accords", icon: ListMusic },
            ].map((tab) => (
              <button
                key={tab.id}
                ...
                className={`flex-1 py-2.5 rounded-lg text-xs font-bold flex items-center justify-center gap-2 ${
                  activeTab === tab.id
                    ? "bg-[#21262d] text-cyan-400 border border-gray-700/50 shadow"
                    : "text-gray-500"
                }`}
              >
            ...
            <div className="animate-in fade-in slide-in-from-top-2 duration-300 space-y-4 bg-[#161b22]/50 p-4 rounded-xl border border-gray-800/30">
              ...
              <Button
                onClick={() =>
                  setChordProgression([
                    ...chordProgression,
                    { id: Date.now(), root: newChordRoot, type: newChordType, name: `${newChordRoot} ${newChordType}` },
                  ])
                }
                variant="success"
                className="w-full"
              ...
              <div className="overflow-x-auto custom-scrollbar-hide flex ml-20 lg:ml-24 mb-4 gap-2">
                {Array.from({ length: drumBars }).map((_, barIdx) => (
                  <div
                    ...
                    style={{ width: "calc((2rem * 4) + (2.5px * 3) + 12px)" }}
                  >
              ...
              <div className="overflow-x-auto pb-6 custom-scrollbar space-y-4">
                {DRUM_TRACKS.map((track, trackIdx) => (
                  <div key={track.id} className="flex items-center gap-4">
                    <div className="w-16 text-[10px] font-black text-gray-400 tracking-tighter uppercase leading-none truncate opacity-80">
                      {track.name}
                    </div>
                          ...
                          {Array.from({ length: 4 }).map((_, stepOffset) => {
                            ...
                            return (
                              <div
                                ...
                                className={`h-11 w-8 lg:w-10 rounded-md cursor-pointer transition-all flex items-center justify-center ${
                                  isActive
                                    ? track.color + " shadow-lg border-transparent"
                                    : beatBg + " border border-gray-800/50"
                                } ${isCurrent ? "ring-2 ring-white scale-105 z-10 brightness-125" : ""} ${
                                  !isActive && isBeatStart ? "border-gray-600/50" : ""
                                }`}
                              >
                                {!isActive && isBeatStart && (
                                  <div
                                    className={`w-1 h-1 rounded-full ${
                                      isMeasureStart ? "bg-gray-400" : "bg-gray-700"
                                    }`}
                                  ></div>
                                )}
                              </div>
                      ...
                      <div key={chord.id} className="relative group">
                        <div
                          className={`w-32 h-32 rounded-2xl border-2 flex flex-col items-center justify-center transition-all ${
                            isPlaying && Math.floor(currentStep / 16) % chordProgression.length === idx
                              ? "border-emerald-500 bg-emerald-500/10 shadow-lg shadow-emerald-500/20 scale-105"
                              : "border-gray-800 bg-[#1a212c]"
                          }`}
                        >
                        ...
                        <button
                          onClick={() => setChordProgression(chordProgression.filter((c) => c.id !== chord.id))}
